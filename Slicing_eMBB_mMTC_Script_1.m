%% Description
% This script generates the curves of eMBB data rate r_B versus mMTC data
% rate r_M for a given number of connected MTC devices.
...
M=10;                                       % Number of mMTC devices connected to the BS
L=32;                                        % Number of receiver antennas
...
Hm_max=(1/sqrt(2))*(randn(L,M,N)+1i*randn(L,M,N));          % Channel coefficients of all the mMTC Devices
...
rm_ort=zeros(1,length(alpha));
parfor k=1:length(alpha)
    for rm=0:0.01:5                                        % Data rate per mMTC device [bits/s/Hz]       
        Dm=zeros(1,N);                                      % Number of correctly decoded mMTC packets in each timeslot
        for j=1:N
            Gm=Gm_max(:,:,j);                               % Select the channel gains of the mMTC devices
...
rm_non=zeros(1,length(rb_non));
parfor x=1:length(rb_non)
    Gm_max_temp=Gm_max;
    gamma_B_temp=gamma_B;
    Gb_temp=Gb;
    gamma_B_min=(2^rb_non(x))-1;
    for rm=0:0.01:5                             % mMTC data rate per device [bits/s/Hz]        
        flag=0;
        for gamma_B_tar=gamma_B_min:0.5:gamma_B_tar_max 
            ...
            Dm=zeros(1,N);                              % Number of correctly decoded mMTC packets in each timeslot
            for j=1:N
                Gm=Gm_max_temp(:,:,j);                  % Select the channel gains of the mMTC devices
                [~,ord]=sort(vecnorm(Gm,2,1).^2,'descend');
                Gm_ord=Gm(:,ord);                               % Sort channel gains of the mMTC devices in descending order
...
rb_non_32=rb_non;
rm_non_32=rm_non;
save('Results_1_L32_M10.mat','rb_ort_32','rm_ort_32','rb_non_32','rm_non_32')

%% Plotting the curves
fig1=figure(1);
    set(fig1,'Position',[200 200 650 500])
    hold on
    plot(rb_ort_16,rm_ort_16,'k--','LineWidth',1.5)
    plot(rb_non_16,rm_non_16,'k-','LineWidth',1.5)    
    grid on
